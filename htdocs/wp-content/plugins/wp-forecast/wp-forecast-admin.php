...
/*  Copyright 2006-2022  Hans Matzen  (email : webmaster at tuxlog dot de)
...
require_once("func_openweathermap.php");
...
{ 
	...
	if(isset($_SERVER['QUERY_STRING'])){
		$wpfqs = $_SERVER['QUERY_STRING'];
	} else {
		$wpfqs = "";
	}
	
	if ( ($wpfqs=="page=wp-forecast-admin.php") && (isset($_POST['info_update']) ))
	{
		for ($i=0;$i<$count;$i++) {
...
function wpf_admin_form($wpfcid='A',$widgetcall=0) 
{
    ...
    // admin hint for use of weather apis
    $out .= '<div class="notice notice-warning"><p>';
    $out .= __('Please notice the following changes are anncounced by the weatherproviders', "wp-forecast_".$locale) . ":<br/>";
	$out .= __('DarkSky will shutdown its free service end of 2022, the service is already closed for new subscriptions.', "wp-forecast_".$locale) . "<br/>";
	$out .= __('accuweather is stopping the open forecastfox api step by step, currently only current weatherdata is still up-to-date.', "wp-forecast_".$locale) . "<br/>";
	$out .= __('wp-forecast added OpenWeatehrMap as a provider to have a good alternative. You can get a free API-Key at.', "wp-forecast_".$locale);
	$out .= '<a target="_blank" href="https://openweathermap.org/price">https://openweathermap.org</a>.';
	$out .= '</p></div>';
...
function wpf_sub_admin_form($wpfcid,$widgetcall) {
	...
	if (isset($_POST['info_update'])) {
		...
		if ($av['ouv_uv']=="") $av['ouv_uv']="0";
     	...
     	//if ($av['ouv_apikey']=="") $av['ouv_apikey']="0";
		if ($av['visitorlocation']=="") $av['visitorlocation']="0";     	   		
...
?>
	 ...
	 <div style="float: left; width: 49%">
         ...
         <select name="service" id="service" size="1" onchange="apifields(document.woptions.service.value);">
           ...
           <option value="openweathermap" <?php if ($av['service']=="openweathermap") echo "selected=\"selected\""?>><?php echo __('OpenWeathermap',"wp-forecast_".$locale)?></option>
	 </select>&nbsp;

         <?php echo __('API-Key',"wp-forecast_".$locale)?>:
