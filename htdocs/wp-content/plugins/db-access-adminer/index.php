...
function adminer_object() {
	...
	$messages = @include Lev0\WpAdminer\INC_MESSAGES;

	class WpAdminer extends Adminer {
...
EOHTML;
	...
	elseif (
		!is_readable(Lev0\WpAdminer\INC_CONDUIT)
		|| !($conduit = @include Lev0\WpAdminer\INC_CONDUIT)
		|| !is_array($conduit)
		|| empty($conduit['url'])
	...
	elseif (empty($_COOKIE[$conduit['cookie_name']])) {
		...
		elseif (
			$status != 200
			|| $body->message !== Lev0\WpAdminer\OK
			|| !$body->creds
		) {
			if (
				$status == 409
		...
		elseif (
			!is_readable(Lev0\WpAdminer\INC_ENCRYPT)
			|| !($encryption = @include Lev0\WpAdminer\INC_ENCRYPT)
			|| !is_array($encryption)
	...
	return new WpAdminer($creds, $message, $auto_submit);
}

if (
	!is_readable(Lev0\WpAdminer\INC_ADMINER)
	|| !include Lev0\WpAdminer\INC_ADMINER
) {
	function h($string) {
