...
function maintain_conduit($auto_submit, $and_messages = false) {
	...
	$time = time();
	return (
			is_readable(INC_CONDUIT)
			&& ($conduit_saved = @include INC_CONDUIT)
...
function maintain_messages() {
	$messages = messages();
	return (
			is_readable(INC_MESSAGES)
			&& ($messages_saved = @include INC_MESSAGES)
			&& is_array($messages_saved)
			&& $messages_saved === $messages
		)
		|| var_file($messages, INC_MESSAGES);
}
...
function maintain_encryption(&$encryption = null) {
	...
	$time = time();
	if (
		!is_readable(INC_ENCRYPT)
		|| !($encryption = @include INC_ENCRYPT)
		|| !is_array($encryption)
		|| $encryption['cipher_algo'] !== $cipher_algo
		|| ($time - $encryption['updated']) >= ENCRYPTION_LIFETIME
	) {
		if (
		...
		if (!var_file($encryption_escaped, INC_ENCRYPT)) {
			return false;
		}
	}
	elseif (
...
function var_file($var, $filename) {
